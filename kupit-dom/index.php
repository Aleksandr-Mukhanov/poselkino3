...
$arFilterPlotsAll = Array("IBLOCK_ID"=>6,"ACTIVE"=>"Y"); // исключим СПБ ,"!PROPERTY_AREA"=>PLOTS_PROP_AREA
...
while ($arElement = $rsElements->Fetch()) { // dump($arElement);
	...
	foreach ($arElement['PROPERTY_VILLAGE_VALUE'] as $value) {
		$arVillageIDs_tags[] = $value;
		$arVillagePlots[$value][] = $arElement['ID'];
	}
}
$arVillageIDs_tags = array_unique($arVillageIDs_tags);
// dump($arVillagePlots);

$newH1 = 'Продажа частных домов в '.REGION_SHORT_WHERE;
$newTitle = 'Купить дом в '.REGION_SHORT_WHERE.' недорого - продажа домов в '.REGION_KOY.' области';
$newDesc = 'Продажа новых домов в поселках '.REGION_SHORT_WHAT.' ➤Цены от '.formatPrice($minPrice).' руб.➤Кол-во объявлений - '.$cnt.' ✔Стоимость коммуникаций ✔Актуальные фото ✔Видео с квадрокоптера ✔Экология местности ✔Отзывы покупателей ✔Юридическая чистота ✔Независимый рейтинг ✔Честный обзор';
...
{
	require_once $_SERVER["DOCUMENT_ROOT"] . '/kupit-dom/inc/seo-filter.php';
	require_once $_SERVER["DOCUMENT_ROOT"] . '/kupit-dom/inc/seo-text.php';
	require_once $_SERVER["DOCUMENT_ROOT"] . '/kupit-dom/inc/seo-multi-tegi.php';
}
...
<main class="page page-va-list">
			...
			<div class="container">
					...
					<div class="show-mobile-filter" style="display: none;">
							<?$APPLICATION->IncludeComponent("bitrix:catalog.smart.filter", "houses", Array(
									...
									"SEF_RULE" => "/kupit-dom/filter/#SMART_FILTER_PATH#/apply/",	// Правило для обработки
			...
			<div class="row align-items-center">
				...
				<div class="col-xl-5 col-lg-6 filter__tab d-none mb-3 mb-md-0 d-sm-block">
					<ul class="nav justify-content-lg-end mt-lg-0 mt-2">
						...
						<li class="nav-item">
							<a class="nav-link btn btn-success rounded-pill" href="<?=htmlspecialcharsbx($urlWithDom)?>">
	...
	<div class="page__content-tag-list">
		<div class="container">
			<?require_once $_SERVER["DOCUMENT_ROOT"].'/kupit-dom/inc/seo-tegi.php';?>
		</div>
