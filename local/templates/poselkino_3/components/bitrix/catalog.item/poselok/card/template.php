...
switch ($km_MKAD) {
    case $km_MKAD <= 10:
        $url_km_MKAD = "do-10-km-ot-".ROAD_URL;
        break;
    case $km_MKAD <= 15:
        $url_km_MKAD = "do-15-km-ot-".ROAD_URL;
        break;
    case $km_MKAD <= 20:
        $url_km_MKAD = "do-20-km-ot-".ROAD_URL;
        break;
    case $km_MKAD <= 25:
        $url_km_MKAD = "do-25-km-ot-".ROAD_URL;
        break;
    case $km_MKAD <= 30:
        $url_km_MKAD = "do-30-km-ot-".ROAD_URL;
        break;
    case $km_MKAD <= 35:
        $url_km_MKAD = "do-35-km-ot-".ROAD_URL;
        break;
    case $km_MKAD <= 40:
        $url_km_MKAD = "do-40-km-ot-".ROAD_URL;
        break;
    case $km_MKAD <= 45:
        $url_km_MKAD = "do-45-km-ot-".ROAD_URL;
        break;
    case $km_MKAD <= 50:
        $url_km_MKAD = "do-50-km-ot-".ROAD_URL;
        break;
    case $km_MKAD <= 55:
        $url_km_MKAD = "do-55-km-ot-".ROAD_URL;
        break;
    case $km_MKAD <= 60:
        $url_km_MKAD = "do-60-km-ot-".ROAD_URL;
        break;
    case $km_MKAD <= 65:
        $url_km_MKAD = "do-65-km-ot-".ROAD_URL;
        break;
    case $km_MKAD <= 70:
        $url_km_MKAD = "do-70-km-ot-".ROAD_URL;
        break;
    case $km_MKAD <= 75:
        $url_km_MKAD = "do-75-km-ot-".ROAD_URL;
        break;
    case $km_MKAD <= 80:
...
foreach ($arDevelopers as $value)
  if (in_array($value,$arResult['DEVELOPERS_TOP100'])) $item['TOP_100'] = true;
?>
<div class="d-flex flex-wrap bg-white card-grid">
    <div class="card-house__photo photo">
      ...
      <div class="slider__header">
        <?if(($item['PROPERTIES']['TOP_100']['VALUE'] || $item['TOP_100']) && $item['PROPERTIES']['SALES_PHASE']['VALUE_XML_ID'] != 'sold'){?>
      ...
      <div class="card-photo__list">
        <?foreach ($itemPhotos as $photo) { // Фото
          ...
          <img class="card-photo__item" src="<?=$photoRes['src']?>" alt="" style="object-fit: cover;">
        <?}?>
        <?if ($showAllPhoto){?>
  				<a class="card-photo__item photo__item-more"  href="<?=$item['DETAIL_PAGE_URL']?>">
      ...
      <div class="photo__count">
        <span class="current">1</span> / <span class="count"><?=$itemPhotosCnt?></span>
      </div>
    </div>
    <div class="card-house__content">
        ...
        <div class="card-house__price mt-1 mt-lg-2">
          <? if ($housesValEnum == PROP_NO_DOM) { // Только участки ?>
            ...
            <div class="card-house__price-number">от <span class="split-number"><?= formatPrice($item['PROPERTIES']['PRICE_SOTKA']['VALUE'][0])?></span> <span class="rub_currency">&#8381;</span></div>
          <? } elseif ($housesValEnum == PROP_WITH_DOM || $housesValEnum == PROP_HOUSE_PLOT) { // Участки с домами ?>
            ...
            <div class="card-house__price-number">от <span class="split-number"><?= formatPrice($item['PROPERTIES']['HOME_VALUE']['VALUE'][0])?></span> <span class="rub_currency">&#8381;</span></div>
          <? } ?>
        </div>
        <hr>
