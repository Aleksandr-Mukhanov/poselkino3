	...
	<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get">
	  ...
	  <?endforeach;?>
		...
		<div class="map-filter__buttons--header">
			...
			<button class="btn btn-outline-warning rounded-pill" id="toggleAreas" type="button">Районы ЛО</button>
		</div>
		<div class="map-filter__select--type mt-2">
			<div class="hide">
				<?foreach($arResult["ITEMS"][225]["VALUES"] as $val => $ar): // dump($ar); // Наличие домов?>
					<input
						type="checkbox"
			...
			<div class="filter__type w-100">
				<select id="filter-type" class="w-100 changeTypeSelect">
					...
					<option class="house-modal" value="arrFilter_2_4088798008" <?if($arResult["ITEMS"][225]["VALUES"][PROP_WITH_DOM]['CHECKED'])echo "selected";?>>Дом</option>
					<option value="arrFilter_2_1842515611" <?if($arResult["ITEMS"][225]["VALUES"][PROP_NO_DOM]['CHECKED'])echo "selected";?>>Участок</option>
				</select>
			</div>
		</div>
        ...
        foreach($arResult["ITEMS"][229]["VALUES"] as $val => $ar){
          ...
          switch ($ar['URL_ID']) {
				...
				<?foreach ($arShosse as $key => $value) {?>
					<div class="col-12">
						<div class="highway-direction text-transform-uppercase font-wetght-bold h4 mt-3 mb-1" id="highwayNorth" data-direction="<?=$key?>" onclick="smartFilter.click(this)"><?=$value['NAME']?></div>
		...
		<div class="map-filter--areas pb-5">
			...
			<h2 class="mt-4 mb-0">Районы ЛО</h2>
			<? // Группируем районы по первым буквам названий
        $prevLetter = '';
        foreach ($arResult["ITEMS"][228]["VALUES"] as $key => $value) {
			...
			<?foreach ($arRegion as $key => $value) {?>
				<div class="row mt-3">
					<div class="col-lg-2 col-xl-1">
						<div class="areas-direction text-transform-uppercase font-wetght-bold h4 mt-2" id="area<?=$key?>" data-direction="<?=$key?>"><?=$key?></div>
					</div>
		...
		<div class="map-filter__content">
			<div class="filter__mkad map-filter__mkad mt-md-2">
				<div class="row align-items-center">
					<?$arItem = $arResult["ITEMS"][231]; //dump($arItem); // Удаленность от МКАД
			...
			<div class="filter__mkad map-filter__mkad mt-md-2">
					...
					<div class="col-9 col-lg-6 col-xl-5  px-0">
						<div class="d-flex justify-content-between">
							<?// $arItem = $arResult["ITEMS"][13]; //dump($arItem); // Стоимость в фильтр
								$arItem = $arResult["ITEMS"][240]; // dump($arItem); // Стоимость участка (в карточку)
								$placeholderMin = ($arItem["VALUES"]["MIN"]["HTML_VALUE"]) ? $arItem["VALUES"]["MIN"]["HTML_VALUE"]: 'От';
								$placeholderMax = ($arItem["VALUES"]["MAX"]["HTML_VALUE"]) ? $arItem["VALUES"]["MAX"]["HTML_VALUE"]: 'До';
							?>
							<input
                ...
                style="margin-right: 0;"
							...
							<input
								...
								/>
							<? // Стоимость домов
								$arItem = $arResult["ITEMS"][244]; // dump($arItem);
								$placeholderMin = ($arItem["VALUES"]["MIN"]["HTML_VALUE"]) ? $arItem["VALUES"]["MIN"]["HTML_VALUE"]: 'От';
								$placeholderMax = ($arItem["VALUES"]["MAX"]["HTML_VALUE"]) ? $arItem["VALUES"]["MAX"]["HTML_VALUE"]: 'До';
							?>
							<input
			...
			<div class="filter__mkad map-filter__mkad mt-md-2">
				<div class="row align-items-center">
					...
					<div class="col-9 col-lg-6 col-xl-5  px-0">
						<div class="d-flex justify-content-between">
							<?$arItem = $arResult["ITEMS"][237]; //dump($arItem); // Площадь участка
							$placeholderMin = ($arItem["VALUES"]["MIN"]["HTML_VALUE"]) ? $arItem["VALUES"]["MIN"]["HTML_VALUE"]: 'от';
							$placeholderMax = ($arItem["VALUES"]["MAX"]["HTML_VALUE"]) ? $arItem["VALUES"]["MAX"]["HTML_VALUE"]: 'до';?>
							<input
								placeholder="<?=$placeholderMin?>"
							...
							<div class="order-3 col-12">
								<div class="extra-options">
									<div class="row align-items-end">
								     ...
								     foreach($arResult["ITEMS"][228]["VALUES"] as $val => $ar){ //dump($ar); // Шоссе
								      if ($ar["CHECKED"]) $activeAreas = 'active';
										...
										<div class="col-xl-3 col-lg-4 mt-40 col-md-5">
											<div class="d-flex align-items-center justify-content-md-end search-group distance-station">
												...
												<?$arItem = $arResult["ITEMS"][292]; //dump($arItem); // Ближайший город расстояние, км?>
												<input
								          data-min-val='<?echo $arItem["VALUES"]["MIN"]["VALUE"]?>'
								          data-max-val='<?echo $arItem["VALUES"]["MAX"]["VALUE"]?>'
								          type="text"
									...
									<div class="row">
										<div class="col-lg-8 mt-40">
									        ...
									        <input
									          ...
									          name="type_permitted"
									          id="dacha"
									          <? echo $arResult["ITEMS"][264]["VALUES"][424]["CHECKED"] ? 'checked="checked"' : '' ?>
									        />
									        ...
									        <input
									          ...
									          name="type_permitted"
									          id="ihs"
									          <? echo $arResult["ITEMS"][264]["VALUES"][433]["CHECKED"] ? 'checked="checked"' : '' ?>
									        />
										      <label class="custom-control-label" data-role="label_ihs" for="ihs">ИЖС</label>
												</div>
												<div class="hide">
													<?foreach($arResult["ITEMS"][264]["VALUES"] as $val => $ar): // Вид разрешенного использования?>
										        <input
										          type="checkbox"
										          value="<? echo $ar["HTML_VALUE"] ?>"
										          name="<? echo $ar["CONTROL_NAME"] ?>"
												...
												<?foreach($arResult["ITEMS"][218]["VALUES"] as $val => $ar): //dump($ar); // Акция?>
													<div class="custom-control custom-checkbox custom-control-inline align-items-center h-100">
														<input
															type="checkbox"
															class="custom-control-input"
															...
															onclick="smartFilter.click(this)"
														/>
														<label class="custom-control-label <? echo $ar["DISABLED"] ? 'disabled': '' ?>" data-role="label_<?=$ar["CONTROL_ID"]?>" for="<? echo $ar["CONTROL_ID"] ?>">Поселки с акциями</label>
													</div>
												<?endforeach;?>
												<?foreach($arResult["ITEMS"][352]["VALUES"] as $val => $ar): //dump($ar); // Рассрочка?>
													<div class="custom-control custom-checkbox custom-control-inline align-items-center h-100">
														<input
															type="checkbox"
															class="custom-control-input"
									...
									<div class="row extra-options__parameters">
										<div class="order-lg-1 col-md-3 col-lg-2 col-sm-4">
											...
											<?foreach($arResult["ITEMS"][250]["VALUES"] as $val => $ar): //dump($ar); // Электричество
								        if($ar["URL_ID"] == 'y'):?>
								          ...
								          />
											...
											<?endif;
								      endforeach;?>
											<?foreach($arResult["ITEMS"][256]["VALUES"] as $val => $ar): //dump($ar); // Водопровод
								        if($ar["URL_ID"] == 'y'):?>
												<div class="extra-options__parameters-input">
													<input
								            type="checkbox"
								          ...
								          />
											...
											<?endif;
								      endforeach;?>
											<?foreach($arResult["ITEMS"][253]["VALUES"] as $val => $ar): //dump($ar); // Газ
								        if($ar["URL_ID"] == 'y'):?>
												<div class="extra-options__parameters-input">
													<input
								            type="checkbox"
								      ...
								      endforeach;?>
										...
										<div class="order-lg-2 col-md-6 col-lg-3 col-xl-2 col-sm-8">
											...
											<div class="row">
												<div class="col-sm-6 col-lg-12">
													<?$i=0;
													foreach($arResult["ITEMS"][306]["VALUES"] as $val => $ar): // dump($ar); // Дороги в поселке
										        if ($ar["URL_ID"] == 'no') continue;
														...
														switch ($ar["URL_ID"]) {
										        	case "asph_crumb": $iconClass = 'asphkr'; break; // Асф. кр.
															case "asphalt": $iconClass = 'asphalt'; break; // Асфальт
										        	case "plate": $iconClass = 'betonniePliti'; break; // Бетонные плиты
															case "brick": $iconClass = 'bitiyKirpich'; break; // Битый кирпич
															case "gravel": $iconClass = 'gruntovka'; break; // Грунтовка
															case "stone": $iconClass = 'sheben'; break; // Щебень
														} $i++;
										        ?>
														<div class="extra-options__parameters-input">
															<input
										            ...
										            onclick="smartFilter.click(this)"
										          />
															<label class="d-flex w-100 align-items-center <? echo $ar["DISABLED"] ? 'disabled': '' ?>" data-role="label_<?=$ar["CONTROL_ID"]?>" for="<? echo $ar["CONTROL_ID"] ?>">
										...
										<div class="order-sm-5 col-sm-8 order-md-5 order-lg-3 col-md-6 col-lg-2">
											<div class="row">
												<div class="col-sm-6 col-lg-12">
													...
													<?foreach($arResult["ITEMS"][301]["VALUES"] as $val => $ar): //dump($ar); // Автобус
										        if($ar["URL_ID"] == 'y'):?>
										        <div class="extra-options__parameters-input">
															<input
										            type="checkbox"
										          ...
										          />
													...
													<?endif;
										      endforeach;?>
													<?foreach($arResult["ITEMS"][296]["VALUES"] as $val => $ar): //dump($ar); // Электричка
										        if($ar["URL_ID"] == 'y'):?>
										        <div class="extra-options__parameters-input">
															<input
										            type="checkbox"
										      ...
										      endforeach;?>
												...
												<div class="col-sm-6 col-lg-12">
													...
													<?foreach($arResult["ITEMS"][308]["VALUES"] as $val => $ar): //dump($ar); // Обустройство поселка
										        if($ar["URL_ID"] != 'no'):
										...
										<div class="order-sm-4 col-sm-4 order-md-3 order-lg-5 order-xl-4 col-md-3 col-lg-2">
											...
											<div class="extra-options__parameters-input">
												<div class="hide">
													<?foreach($arResult["ITEMS"][281]["VALUES"] as $val => $ar): //dump($ar); // Водоем
									        if($ar["URL_ID"] != 'no'):?>
								          <input
								            type="checkbox"
								            value="<? echo $ar["HTML_VALUE"] ?>"
									      ...
									      <?endif;
									      endforeach;?></div>
												<input class="sr-only iconDouble" type="checkbox" id="vodoem" data-id1="<?=$arResult["ITEMS"][281]["VALUES"][459]["CONTROL_ID"]?>" data-id2="<?=$arResult["ITEMS"][281]["VALUES"][460]["CONTROL_ID"]?>" data-id3="<?=$arResult["ITEMS"][281]["VALUES"][461]["CONTROL_ID"]?>">
											...
											<div class="extra-options__parameters-input">
												<div class="hide">
													<?foreach($arResult["ITEMS"][279]["VALUES"] as $val => $ar): //dump($ar); // Лес
									        if($ar["URL_ID"] != 'no'):?>
								          <input
								            type="checkbox"
								            value="<? echo $ar["HTML_VALUE"] ?>"
									      ...
									      <?endif;
									      endforeach;?></div>
												<input class="sr-only iconDouble" type="checkbox" id="les" data-id1="<?=$arResult["ITEMS"][279]["VALUES"][454]["CONTROL_ID"]?>" data-id2="<?=$arResult["ITEMS"][279]["VALUES"][455]["CONTROL_ID"]?>" data-id3="<?=$arResult["ITEMS"][279]["VALUES"][457]["CONTROL_ID"]?>" data-id4="<?=$arResult["ITEMS"][279]["VALUES"][458]["CONTROL_ID"]?>">
											...
											<?foreach($arResult["ITEMS"][283]["VALUES"] as $val => $ar): //dump($ar); // Пляж для купания
								        if($ar["URL_ID"] == 'y'):?>
												<div class="extra-options__parameters-input">
													<input
								            type="checkbox"
										...
										<div class="order-sm-3 col-sm-8 order-md-4 order-lg-4 order-xl-5 col-md-6 col-lg-3 col-xl-4">
											...
											<div class="row">
												<div class="col-sm-6 col-lg-12 col-xl-6">
													<div class="extra-options__parameters-input">
														<div class="hide"><?foreach($arResult["ITEMS"][309]["VALUES"] as $val => $ar): //dump($ar); // Магазин?>
															<input
																type="checkbox"
																value="<? echo $ar["HTML_VALUE"] ?>"
																name="<? echo $ar["CONTROL_NAME"] ?>"
															...
															/> <?=$ar["VALUE"];?>
											      <?endforeach;?></div>
														<input class="sr-only iconDouble" type="checkbox" id="magazin" data-id1="<?=$arResult["ITEMS"][309]["VALUES"][496]["CONTROL_ID"]?>" data-id2="<?=$arResult["ITEMS"][309]["VALUES"][497]["CONTROL_ID"]?>">
													...
													<div class="extra-options__parameters-input">
														<div class="hide"><?foreach($arResult["ITEMS"][311]["VALUES"] as $val => $ar): //dump($ar); // Церковь?>
															<input
																type="checkbox"
																value="<? echo $ar["HTML_VALUE"] ?>"
																name="<? echo $ar["CONTROL_NAME"] ?>"
															...
															/> <?=$ar["VALUE"];?>
											      <?endforeach;?></div>
														<input class="sr-only iconDouble" type="checkbox" id="cerkov" data-id1="<?=$arResult["ITEMS"][311]["VALUES"][502]["CONTROL_ID"]?>" data-id2="<?=$arResult["ITEMS"][311]["VALUES"][503]["CONTROL_ID"]?>">
													...
													<div class="extra-options__parameters-input">
														<div class="hide"><?foreach($arResult["ITEMS"][312]["VALUES"] as $val => $ar): //dump($ar); // Школа?>
															<input
																type="checkbox"
																value="<? echo $ar["HTML_VALUE"] ?>"
																name="<? echo $ar["CONTROL_NAME"] ?>"
															...
															/> <?=$ar["VALUE"];?>
											      <?endforeach;?></div>
														<input class="sr-only iconDouble" type="checkbox" id="school" data-id1="<?=$arResult["ITEMS"][312]["VALUES"][505]["CONTROL_ID"]?>" data-id2="<?=$arResult["ITEMS"][312]["VALUES"][506]["CONTROL_ID"]?>">
													...
													<div class="extra-options__parameters-input">
														<div class="hide"><?foreach($arResult["ITEMS"][313]["VALUES"] as $val => $ar): //dump($ar); // Дет.сад?>
															<input
																type="checkbox"
																value="<? echo $ar["HTML_VALUE"] ?>"
																name="<? echo $ar["CONTROL_NAME"] ?>"
															...
															/> <?=$ar["VALUE"];?>
											      <?endforeach;?></div>
														<input class="sr-only iconDouble" type="checkbox" id="detsad" data-id1="<?=$arResult["ITEMS"][313]["VALUES"][508]["CONTROL_ID"]?>" data-id2="<?=$arResult["ITEMS"][313]["VALUES"][509]["CONTROL_ID"]?>">
												...
												<div class="col-sm-6 col-lg-12 col-xl-6">
													<div class="extra-options__parameters-input">
														<div class="hide"><?foreach($arResult["ITEMS"][315]["VALUES"] as $val => $ar): // dump($val); // Кафе?>
															<input
																type="checkbox"
																value="<? echo $ar["HTML_VALUE"] ?>"
																name="<? echo $ar["CONTROL_NAME"] ?>"
															...
															/> <?=$ar["VALUE"];?>
											      <?endforeach;?></div>
														<input class="sr-only iconDouble" type="checkbox" id="cafe" data-id1="<?=$arResult["ITEMS"][315]["VALUES"][514]["CONTROL_ID"]?>" data-id2="<?=$arResult["ITEMS"][315]["VALUES"][515]["CONTROL_ID"]?>">
													...
													<div class="extra-options__parameters-input">
														<div class="hide"><?foreach($arResult["ITEMS"][316]["VALUES"] as $val => $ar): //dump($ar); // АЗС?>
															<input
																type="checkbox"
																value="<? echo $ar["HTML_VALUE"] ?>"
																name="<? echo $ar["CONTROL_NAME"] ?>"
															...
															/> <?=$ar["VALUE"];?>
											      <?endforeach;?></div>
														<input class="sr-only iconDouble" type="checkbox" id="azs" data-id1="<?=$arResult["ITEMS"][316]["VALUES"][517]["CONTROL_ID"]?>" data-id2="<?=$arResult["ITEMS"][316]["VALUES"][518]["CONTROL_ID"]?>">
													...
													<div class="extra-options__parameters-input">
														<div class="hide"><?foreach($arResult["ITEMS"][310]["VALUES"] as $val => $ar): //dump($ar); // Аптека?>
															<input
																type="checkbox"
																value="<? echo $ar["HTML_VALUE"] ?>"
																name="<? echo $ar["CONTROL_NAME"] ?>"
															...
															/> <?=$ar["VALUE"];?>
											      <?endforeach;?></div>
														<input class="sr-only iconDouble" type="checkbox" id="apteka" data-id1="<?=$arResult["ITEMS"][310]["VALUES"][499]["CONTROL_ID"]?>" data-id2="<?=$arResult["ITEMS"][310]["VALUES"][500]["CONTROL_ID"]?>">
													...
													<div class="extra-options__parameters-input">
														<div class="hide"><?foreach($arResult["ITEMS"][314]["VALUES"] as $val => $ar): //dump($ar); // Строймат?>
															<input
																type="checkbox"
																value="<? echo $ar["HTML_VALUE"] ?>"
																name="<? echo $ar["CONTROL_NAME"] ?>"
															...
															/> <?=$ar["VALUE"];?>
											      <?endforeach;?></div>
														<input class="sr-only iconDouble" type="checkbox" id="stroimat" data-id1="<?=$arResult["ITEMS"][314]["VALUES"][511]["CONTROL_ID"]?>" data-id2="<?=$arResult["ITEMS"][314]["VALUES"][512]["CONTROL_ID"]?>">
		        ...
		        <div class="row">
		          <div class="col-12">
		            <div class="d-flex flex-wrap">
									<?foreach($arResult["ITEMS"][340]["VALUES"] as $val => $ar): //dump($ar); // Материал домов?>
										<div class="checkbox-house">
											<input
												...
												class="sr-only"
											/>
											<label class="radio <? echo $ar["DISABLED"] ? 'disabled': '' ?>" data-role="label_<?=$ar["CONTROL_ID"]?>" for="<? echo $ar["CONTROL_ID"] ?>"><?=$ar["VALUE"];?></label>
										</div>
					        <?endforeach;?>
		            </div>
		          </div>
		            ...
		            <div class="modal-area-home">
		              <div class="modal-area-home__input">
		                <div class="d-flex align-items-center">
											<?$arItem = $arResult["ITEMS"][242]; //dump($arItem); // Площадь домов?>
		          ...
		          <div class="col-lg-7 col-md-6 mt-3 mt-md-0">
		            <div class="d-flex flex-wrap">
									<?foreach($arResult["ITEMS"][342]["VALUES"] as $val => $ar): //dump($ar); // Дома с отделкой?>
										<div class="custom-control custom-checkbox custom-control-inline align-items-center h-100">
											<input
				                type="checkbox"
				                value="<? echo $ar["HTML_VALUE"] ?>"
				          ...
				          <?endforeach;?>
							...
							<?if($arResult["ITEMS"][341]["VALUES"]):?>
		          <div class="col-xl-12 mt-4 mb-4">
		            <p class="mb-0 text-secondary">Готовность:</p>
		            <div class="d-flex flex-wrap">
									<?foreach($arResult["ITEMS"][341]["VALUES"] as $val => $ar): //dump($ar); // Стадия готовности?>
										<div class="custom-control custom-checkbox custom-control-inline align-items-center h-100">
											<input
				                type="checkbox"
				                value="<? echo $ar["HTML_VALUE"] ?>"
