    ...
    if ($arVillageInfo[0])
		{
			...
			if ($arVillageInfo[0] == 'Южные озёра 2') $arVillageInfo[0] = 'Южные озера-2';
			if ($arVillageInfo[0] == 'Южные озёра 3') $arVillageInfo[0] = 'Южные озера-3';
			if ($arVillageInfo[0] == 'Заповедный берег 2') $arVillageInfo[0] = 'Заповедный берег-2';
			if ($arVillageInfo[0] == 'Васильково 4') $arVillageInfo[0] = 'Васильково-4 (вторая очередь)';
			if ($arVillageInfo[0] == 'Сосновый бор') $arVillageInfo[0] = 'Сосновый бор 1,2';
			if ($arVillageInfo[0] == 'Великие озёра') $arVillageInfo[0] = 'Великие Озёра';
			if ($arVillageInfo[0] == 'Долина озёр 2') $arVillageInfo[0] = 'Долина Озер 2';
...
if ($arVillageNames)
{
  ...
  while($arElement = $rsElements->Fetch())
	...
	foreach ($arPlotFeed as $plotFeed) // перебираем участки из фида
	{
		...
		if ($villageCode == 'velikie-ozyera') $villageCode = 'velikie_ozera';
		if ($villageCode == 'dolina-ozyer-2') $villageCode = 'dolina-ozer-2';
		if ($villageCode == 'yuzhnye-ozyera-2') $villageCode = 'yuzhnye-ozera-2';
		if ($villageCode == 'yuzhnye-ozyera-3') $villageCode = 'yuzhny-ozera-3';
		if ($villageCode == 'vasilkovo-4') $villageCode = 'vasilkovo-4-2';
		if ($villageCode == 'sosnovyy-bor') $villageCode = 'sosnovyy-bor-1-2';
	...
	foreach ($arVillageOur as $ourVillage) // перебираем наши поселки
	{
		...
		// dump($ourVillage);dump($feedVillage);
		if(strtotime($feedVillage['DATE_UPDATE']) > strtotime($ourVillage['DATE_FEED'])) // если обновлен
		{
			...
			$cntPlotsSold = $ourVillage['COUNT_PLOTS'] - $ourVillage['COUNT_PLOTS_SALE'];
			if($cntPlotsSold && ($cntPlotsSold != $ourVillage['COUNT_PLOTS_SOLD']));
			{
				if (!$test)
					CIBlockElement::SetPropertyValues($villageID, 1, $cntPlotsSold, "COUNT_PLOTS_SOLD");
