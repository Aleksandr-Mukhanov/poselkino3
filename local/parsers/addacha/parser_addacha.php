...
const SITE_NAME = 'addacha'; //Название сайта для получения настроек конкретного парсера из config.json
...
$villages = $main_page->query('//div[@id = "poselok-list"]/div[@class="col-md-3 col-sm-3 col-xs-7"]/div[@class="itemWrapper"]');
if ($villages->length == 0)
    $parser->reg_parse_error('villages not found', 'Парсер не обнаружил поселков.');
...
foreach($villages as $village_num => $village_inf)
{
    ...
    $village_link = $parser->base_site_link.($main_page->query('div[@class="itemImage"]/a', $village_inf)[0]->getAttribute('href'));
    ...
    $parser->add_village_info('name', $main_page->query('div[@class="itemName"]/h3/a', $village_inf)[0]->textContent);
    $parser->add_village_info('plots_count', $village_page->query('//div[@class="col-md-5 col-sm-5 params img-rounded info"]/ul[2]/li[5]/span')[0]->textContent);

    //Получение страницы участков поселка
    $file_contest = $parser->open_plot_page($parser->base_site_link.($village_page->query('//ul[@id = "menu"]/li[3]/a')[0]->getAttribute('href')));
    if (!$file_contest)
       continue;
    ...
    //Получение страницы фотогалереи
    $photos_dom = null;
    $photos = null;
    $file_contest = file_get_contents("{$village_link}fotografii-poselka/#menu");
    if (!$file_contest)
        $parser->reg_parse_error('gallery not found', "Не удалось получить галерею поселка ".$parser->getVillageProperty('name')." по ссылке $village_link/fotografii-poselka/#menu");
    else
    {
        $photos_dom = new DOMDocument();
        $photos_dom->loadHTML($file_contest);
        $photos = new DOMXPath($photos_dom);
    }

    $buf = $photos->query('//div[@class = "col-md-12"]/a');
    if ($buf->length > 0)
        foreach ($buf as $village_link_add)
            $parser->add_village_info('link', $parser->base_site_link.$village_link_add->getAttribute('href'));
    else
        $parser->reg_parse_error('gallery images not found', "Изображения в галерее не найдены - поселок ".$parser->getVillageProperty('name')." по ссылке {$village_link}fotografii-poselka/#menu");

    output_information("\nНачат парсинг поселка ".$parser->getVillageProperty('name').', всего в нем '.$parser->getVillageProperty('plots_count').' участков', true, true, 'village');
    foreach($plots_page->query('//table[@class="filter table table-bordered table-striped"]/tbody/tr') as $plot)
    {
        set_time_limit(20); //Добавление времени работы скрипта с каждым участком
        //Получение информации об участке

        //Номер участка
        $num = $plots_page->query('td[4]', $plot)[0]->textContent;

        $parser->add_plot_info('num_id', $num);
        $parser->add_plot_info('price', $plots_page->query('td[7]', $plot)[0]->textContent);
        $parser->add_plot_info('area', $plots_page->query('td[6]', $plot)[0]->textContent);
    ...
    function add_plot_info($attribute, $info)
    {
        switch ($attribute)
        {
            case 'name':
                ...
                break;
            case 'num_id':
                $this->plot_arr['num_id'] = preg_replace('/(\D\D)/', '', $info);
                break;
            case 'price':
                if ($info == null || $info == ''){
