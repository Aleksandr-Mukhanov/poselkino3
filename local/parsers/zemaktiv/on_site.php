...
$parserName = 'zemaktiv';
$siteName = 'zemaktiv.ru';
$xmlDeveloper = 'ZemAktiv';
    ...
    if ($arVillageInfo[0])
		{
			...
			if ($arVillageInfo[0] == 'Коськово') $arVillageInfo[0] = 'Коськово (Первая очередь)';
			if ($arVillageInfo[0] == 'Берег Вяземскиx') $arVillageInfo[0] = 'Берег Вяземских';
...
if ($arVillageNames)
{
  ...
  while($arElement = $rsElements->Fetch())
	...
	foreach ($arPlotFeed as $plotFeed) // перебираем участки из фида
	{
		...
		if ($villageCode == 'koskovo') $villageCode = 'koskovo-';
		if ($villageCode == 'bereg-vyazemskix') $villageCode = 'bereg-vyazemskikh';
