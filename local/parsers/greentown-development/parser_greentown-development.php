...
const SITE_NAME = 'greentown-development';  //Название сайта для получения настроек конкретного парсера из config.json
...
$dom = new DOMDocument();
$dom->loadHTML($parser->open_main_page("{$parser->base_site_link}/villages/"));
$main_page = new DOMXPath($dom);

//Получение всех поселков страницы
$villages = $main_page->query('//a[@class = "villages__item tab-pane"]');

foreach($villages as $li)
{
    ...
    $village_page = new DOMXPath($village_dom);
     ...
     //Обнаружение скрипта со списком участков
    $plots =  explode('let', $village_page->query('//script')[15]->textContent);
    if (array_key_exists(1,  $plots))
    {
        $plots = substr($plots[1], 10, -9);
        $plots = preg_replace("/[']/", '"', $plots);
    ...
    $parser->add_village_info('name', $li->childNodes[3]->childNodes[1]->childNodes[1]->textContent);
    $parser->add_village_info('plots_count', sizeof($plots));

    //Получение изображений в галерее
    $buf = $village_page->query('//img[@class = "village-detail__gallery-item"]');
    if ($buf->length > 0)
    ...
    foreach($plots as $plot)
    {
        ...
        if ($plot['sold'] != 'NO' || $plot['reserved'] != 'NO')
            continue;

        $num = preg_replace('/[^0-9]/', '', $plot['id']);

        //Создание dom и xpath данного участка
        $plot_dom = new DOMDocument();
        $plot_dom->loadHTML($plot['info']);
        $plot_page = new DOMXPath($plot_dom);

        $price = $plot_page->query('//b');
        if ($price->length == 0)
        {
            if ($configuration['use_logs'])
                reg_parse_error('price not found', "Список характеристик не найден - участок $num в ".$village_arr['village_name']);
            continue;
        }

        $parser->add_plot_info('num_id', $num);
        $parser->add_plot_info('price', $price[1]->textContent);
        $parser->add_plot_info('area', $price[0]->textContent);
