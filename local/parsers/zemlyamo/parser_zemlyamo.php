...
$dom = new DOMDocument();
$dom->loadHTML($parser->open_main_page("$parser->base_site_link/village"));
...
foreach($main_page->query('//div[@class="sale-box__content"]/div[@class = "link"]/a') as $li)
{
    ...
    $li_text = preg_replace('/^\s+|\s+$|\s+(?=\s)/', '', $li->textContent);
    ...
    if ($li_text == 'Подробнее')
    {
        ...
        $name = explode('«', $village_page->query('//div[@class = "h3 mb-0 wow fadeInUp"]')[0]->textContent)[1];
        $parser->add_village_info('name', substr($name, 0, -2));
    }
    else if ($li_text == 'Сайт посёлка')
    {
        ...
        {
            $file_contest = $parser->open_village_page('https://kp-river.ru/river-park');
            if (!$file_contest)
                continue;
            ...
            $parser->add_village_info('name', "River Park");
        ...
        else
        {
            ...
            if (!$file_contest)
            {
                $parser->reg_parse_error('village not found', "Не удалось получить поселок со ссылкой $village_link.");
                continue;
            }
            ...
            //Определение ссылки на генплан, имени поселка и xpath query для поиска изображений в галерее
            $parser->add_village_info('name', $village_page->query('//div[@class = "col-12"]/h2[@class="mb-6"]')[0]->textContent);
    ...
    if (!$file_contest)
    {
        $parser->reg_parse_error('genplan not found', "Не удалось получить генплан поселка со ссылкой '$village_link' по ссылке '$genplan_link'.");
        continue;
    }
    ...
    {
        $parser->reg_parse_error('plots not found', "В поселке по ссылке $village_link на генплане не обнаружены участки");
        continue;
    }
    ...
    if (sizeof($buf) > 0)
        foreach ($buf as $village_link_add)
            if ($village_link_add->getAttribute('href'))
                $parser->add_village_info('link', ($another_site ? $village_link : $parser->base_site_link).$village_link_add->getAttribute('href'));
    ...
    foreach($plots as $plot)
    {
        ...
        if (!$num || $plot['status'] != 'Свободен')
            continue;

        $parser->add_plot_info('num_id', $num); //Номер участка
        $parser->add_plot_info('price', $plot['objectprice']);
        $parser->add_plot_info('area', $plot['square_value']);
