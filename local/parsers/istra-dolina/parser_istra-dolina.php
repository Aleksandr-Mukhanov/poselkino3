...
const SITE_NAME = 'istra-dolina'; //Название сайта для получения настроек конкретного парсера из config.json
...
$dom = new DOMDocument();
$dom->loadHTML($parser->open_main_page("$parser->base_site_link/poselki/"));
$main_page = new DOMXPath($dom);

//Получение всех поселков страницы
$villages = $main_page->query('//div[@class="innerct"]/a');
foreach($villages as $village_num => $li)
{
    ...
    foreach ($village_page->query('//script') as $script)
        if (str_contains($script->textContent, 'var poligon={'))
        {
            $beg = strpos($script->textContent, 'var poligon') + strlen('var poligon=');
            $end = strpos($script->textContent, ';', -500);
            $plots = preg_replace("/'/", '"', substr($script->textContent, $beg, $end - $beg));
            $plots = json_decode($plots, true);
            $parser->add_village_info('plots_count',  sizeof($plots)); //Запись количества участков в поселке
            $json_array = true;
        }
    if ($plots == null){
        $plots = $village_page->query('//polygon');
        $parser->add_village_info('plots_count', $plots->length/2); //Запись количества участков в поселке
        $json_array = false;
    }

    //Запись имени поселка
    $parser->add_village_info('name', $main_page->query('div[@class = "shg"]/div[@class = "dop2"]/div[@class = "dw33"]', $li)[0]->textContent);

    //Получение изображений в галерее
    $buf = $village_page->query('//div[@class="gal_dom_one"]/a');
    if ($buf->length > 0)
        foreach ($buf as $village_link_add)
            $parser->add_village_info('link', $parser->base_site_link.$village_link_add->getAttribute('href'));
    else
        $parser->reg_parse_error('gallery images not found', "Изображения в галерее не найдены - поселок ".$parser->getVillageProperty('name'));
    ...
    foreach($plots as $plot)
    {
        ...
        if (plot_inf($plot, 'status') == 2 || !((plot_inf($plot, 'status') >= 1 && plot_inf($plot, 'status') <= 5) || plot_inf($plot, 'status') == 7))
            continue;

        $num = plot_inf($plot, 'num');

        $parser->add_plot_info('price', plot_inf($plot, plot_inf($plot, 'discount_price') == '' ? 'price' : 'discount_price'));
        $parser->add_plot_info('num_id', $num);
        $parser->add_plot_info('area', plot_inf($plot, 'place'));
...
function plot_inf(&$plot, $attr)
{
    global $json_array;
    return $json_array ? $plot[$attr] : $plot->getAttribute("data-$attr");
}
