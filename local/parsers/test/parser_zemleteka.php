...
const SITE_NAME = 'zemleteka'; //Название сайта для получения настроек конкретного парсера из config.json
...
$dom = new DOMDocument();
$dom->loadHTML($parser->open_main_page("$parser->base_site_link/projects.html"));
$main_page = new DOMXPath($dom);

//Получение всех поселков страницы
$villages = $main_page->query('//div[@class="townships-catalog__sale-items"]/a[@class="free-plot__slide"]');

foreach($villages as $village_num => $village_inf)
{
    ...
    $village_link = $parser->base_site_link.($village_inf->getAttribute('href'));
    $file_contest = $parser->open_village_page($village_link);
    $file_contest = mb_convert_encoding($file_contest, 'HTML-ENTITIES', 'UTF-8');
    if (!$file_contest)
        continue;
    ...
    $village_chars = $main_page->query('div[@class="free-plot__slide-body"]', $village_inf)[0];
    $name = explode('«', $village_page->query('//div[@class="township__topinfo"]/h2')[0]->textContent);
    $parser->add_village_info('name', preg_replace('/»/', '', $name[sizeof($name)-1]));

    $plots_count = $main_page->query('div[@class="free-plot__slide-text"]', $village_chars)[0]->childNodes[2];
    $parser->add_village_info('plots_count', $plots_count->textContent);
    
    //Получение галлереи
    $gallery = $village_page->query('//div[@class="gallery-township__wrapper swiper-wrapper"]/div/div/img');
    foreach ($gallery as $img)
        $parser->add_village_info('link', $parser->base_site_link.$img->getAttribute('src'));

    $village_id = $village_page->query('//input[@id="docid"]')[0]->getAttribute('value');

    $file_contest = $parser->open_plot_page("https://zemleteka.ru/tablicau.html?document=$village_id&param_id=3&direction=up");
    if (!$file_contest)
        continue;
    $plot_dom = new DOMDocument;
    $plot_dom->loadHTML($file_contest);
    $plot_xpath = new DOMXPath($plot_dom);

    output_information("\nНачат парсинг поселка ".$parser->getVillageProperty('name').', всего в нем '.$parser->getVillageProperty('plots_count').' участков', true);
    foreach($plot_xpath->query('//ul') as $plot)
    {
        set_time_limit(20); //Добавление времени работы скрипта с каждым участком
        $plot = $plot_xpath->query('li', $plot);
        if ($plot[0]->textContent == ' ')
            continue;

        $parser->add_plot_info('num_id', $plot[0]->textContent);
        $parser->add_plot_info('price', $plot[3]->textContent);
        $parser->add_plot_info('area', $plot[1]->textContent);
...
class Parser
{
    ...
    function __construct($configuration)
    {
        ...
        $this->village_filename = $this->configuration[SITE_NAME]['village_filename'];
        $this->plots_filename = $this->configuration[SITE_NAME]['plots_filename'];
    ...
    function add_plot_info($attribute, $info)
    {
        switch ($attribute)
        {
            case 'name':
                ...
                $this->plot_arr['name'] = preg_replace('/[\n\t]/', '', $info);
                $this->plot_arr['name'] = preg_replace('/(  )/', '', $this->plot_arr['name']);
                break;

            case 'num_id':
                $this->plot_arr['num_id'] = preg_replace('/[\n\t]/', '', $info);
                $this->plot_arr['num_id'] = preg_replace('/(  )/', '', $this->plot_arr['num_id']);
                break;
            case 'price':
                if ($info == null || $info == ''){
    ...
    function add_village_info($attribute, $info)
    {
        switch ($attribute)
        {
            case 'name':
                ...
                $this->village_arr['name'] = preg_replace('/[\n\t]/', '', $info);
                $this->village_arr['name'] = preg_replace('/(  )/', '', $this->village_arr['name']);
        ...
        if (sizeof($this->customs['buff_plots']) > 0)
        {
            ...
            {    
                if ($this->plot_arr['num_id'] == $saved_value[2] && $this->plot_arr['name'] == $saved_value[0])
                {
                    ...
                    if (sizeof($diff) > (array_key_exists('links', $diff) + array_key_exists('last_update', $diff)))
    ...
    function dump_village_info()
    {
        ...
        if ($this->village_arr['plots_on_sale'] == 0 && !$this->configuration[SITE_NAME]['keep_zero_villages'])
        {
            ...
            if ($this->configuration[SITE_NAME]['output_warnings'])
            {
                ...
                {
                    output_warning('В поселке '.$this->village_arr['name'].' не найдены участки в продаже, поселок не записан. Чтобы оставлять такие поселки, поменяйте в файле config.json для '.SITE_NAME.' настройку keep_zero_villages на true.');
    ...
    private function reg_parse_error($error_name, $description) //Добавляет ошибку в массив $parser_errores и записывает её в файл logs.txt
    {
    ...
    private function reg_parse_warning($warning_name, $description)
    {
        ...
        {
            if ($this->configuration[SITE_NAME]['use_logs'])
                file_put_contents('logs.txt', date('Y-m-d H:i:s').": $warning_name - $description\n", FILE_APPEND);
        }
