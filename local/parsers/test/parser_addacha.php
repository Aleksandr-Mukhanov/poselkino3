...
foreach($villages as $village_num => $village_inf)
{
    ...
    $village_link = $base_site_link.($main_page->query('div[@class="itemImage"]/a', $village_inf)[0]->getAttribute('href'));
    ...
    $village_arr['village_name'] = $main_page->query('div[@class="itemName"]/h3/a', $village_inf)[0]->textContent;
    $village_arr['plots_count'] = $village_page->query('//div[@class="col-md-5 col-sm-5 params img-rounded info"]/ul[2]/li[5]/span')[0]->textContent;
    ...
    $file_contest = file_get_contents($base_site_link.($village_page->query('//ul[@id = "menu"]/li[3]/a')[0]->getAttribute('href')));
    if (!$file_contest)
    {
        reg_parse_error('plots not found', "Не удалось получить участки поселка ".$village_arr['village_name'].' по ссылке '.$base_site_link.($village_page->query('//ul[@id = "menu"]/li[3]/a')[0]->getAttribute('href')));
        continue;
    }
    ...
    if (!$file_contest)
        reg_parse_error('gallery not found', "Не удалось получить галерею поселка ".$village_arr['village_name']." по ссылке $village_link/fotografii-poselka/#menu");
    else
    {
    ...
    foreach($plots_page->query('//table[@class="filter table table-bordered table-striped"]/tbody/tr') as $plot)
    {
        ...
        //Цена участка
        $plots_arr['price'] = $plots_page->query('td[7]', $plot)[0]->textContent;
        if ($plots_arr['price'] != null)
            $plots_arr['price'] = (int)preg_replace('/[^0-9]/', '', $plots_arr['price']);
        ...
        $plots_arr['area'] = $plots_page->query('td[6]', $plot)[0]->textContent;
        if ($plots_arr['area'] != null)
            $plots_arr['area'] = (float)preg_replace('/,/', '.', $plots_arr['area']);
        else if ($use_logs || $output_errores)
        ...
        if ($plots_arr['area'] != null && $plots_arr['price'] )
        ...
        if ($output_plots)
            show_plot_information($village_link);
    }
...
function show_plot_information($plot_link) //Отобразить данные о текущем участке
{
    switch (PHP_SAPI)
    {
    case 'cli':
        ...
        break;
    default:
        echo "<br/><span> Получен участок: <a href = \"{$plot_link}\">{$plot_link}</a></span>";
        break;
    } 
}
