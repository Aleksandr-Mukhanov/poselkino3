...
foreach($villages as $li)
{
    ...
    if ($plots == null || sizeof($plots) == 0)
    {
        if ($use_logs || $output_errores)
            reg_parse_error('page_not_found', "Участки поселка по ссылке $village_link не найдены.");
        continue;
    }

    //Запись имени и количества участков данного поселка
    $village_arr['village_name'] = preg_replace('/[\t\n]/', '', $li->childNodes[3]->childNodes[1]->childNodes[1]->textContent);
    $village_arr['village_name'] = preg_replace('/(  )/', '', $village_arr['village_name']);
    ...
    foreach($plots as $plot)
    {
        ...
        //Номер участка
        $plots_arr['num_id'] = preg_replace('/[^0-9]/', '', $plot['name']);

        //Цена участка
        $plots_arr['price'] = $price[1]->textContent;
        ...
        $plots_arr['area'] = $price[0]->textContent;
