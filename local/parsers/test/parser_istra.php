...
$file_contest = file_get_contents("$base_site_link/poselki/");
...
foreach($villages as $village_num => $li)
{
    ...
    foreach ($village_page->query('//script') as $script)
        if (str_contains($script->textContent, 'var poligon={'))
        {
            ...
            $village_arr['plots_count'] = sizeof($plots); //Запись количества участков в поселке
            $json_array = true;
        }
    if ($plots == null){
        $plots = $village_page->query('//polygon');
        $village_arr['plots_count'] = $plots->length/2; //Запись количества участков в поселке
        $json_array = false;
    }

    //Запись имени поселка
    $village_arr['village_name'] = preg_replace('/[\n\t]/', '', $main_page->query('div[@class = "shg"]/div[@class = "dop2"]/div[@class = "dw33"]', $li)[0]->textContent);
    ...
    foreach($plots as $plot)
    {
        ...
        //Цена участка
        $plots_arr['price'] = (int)plot_inf($plot, plot_inf($plot, 'discount_price') == '' ? 'price' : 'discount_price');
        if ($plots_arr['price'] == 0 && ($configuration[SITE_NAME]['use_logs'] || $output_errores))
        ...
        $plots_arr['area'] = plot_inf($plot, 'place');
        ...
        if ($plots_arr['area'] != null && $plots_arr['price'] != null)
