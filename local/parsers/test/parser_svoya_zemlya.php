...
const SITE_NAME = 'svoya-zemlya'; //Название сайта для получения настроек конкретного парсера из config.json
...
$dom = new DOMDocument();
$dom->loadHTML($parser->open_main_page("$parser->base_site_link/catalog/dachi/"));
$main_page = new DOMXPath($dom);

//Получение всех поселков страницы
$villages = $main_page->query('//div[@class="col-12 col-md-6 col-lg-4 removedItem-js"]/a');
...
foreach($villages as $village_num => $village_inf)
{
    ...
    $village_chars = $village_page->query('//div[@class="sdc-right col-12 col-xl-6"]/div[@class="sdc-row"]');
    $parser->add_village_info('name', $village_page->query('//div[@class="sd-top container"]/h1/span')[0]->textContent);
    $parser->add_village_info('plots_count', $village_chars[6]->textContent);
    
    //Получение страницы участков поселка
    $file_contest = $parser->open_plot_page("{$village_link}price/");
    if (!$file_contest)
        continue;

    $plots_dom = new DOMDocument();
    $plots_dom->loadHTML($file_contest);
    $plots_page = new DOMXPath($plots_dom);
    //Получение галлереи
    $gallery = $village_page->query('//div[@class="sds-hidden-photos"]/a');
    foreach ($gallery as $img)
        $parser->add_village_info('link', $parser->base_site_link.$img->getAttribute('href'));

    output_information("\nНачат парсинг поселка ".$parser->getVillageProperty('name').', всего в нем '.$parser->getVillageProperty('plots_count').' участков', true);
    foreach($plots_page->query('//tr[position()>1]') as $plot)
    {
        set_time_limit(20); //Добавление времени работы скрипта с каждым участком
        if ($plots_page->query('td[6]', $plot)[0]->textContent != 'Свободен')
            continue;

        $parser->add_plot_info('num_id', (int)$plots_page->query('td[1]/a', $plot)[0]->getAttribute('data-value'));

        $price = $plots_page->query('td[4]/a', $plot)[0]->childNodes;
        $parser->add_plot_info('price', $price[$price->length - 3]->textContent);

        $parser->add_plot_info('area', $plots_page->query('td[3]/a', $plot)[0]->getAttribute('data-value'));
        $parser->add_plot_info('cadastral_number', $plots_page->query('td[5]/a', $plot)[0]->textContent);
