...
$file_contest = file_get_contents("{$base_site_link}/genplan.html");
...
foreach($main_page->query('//div[@class = "row"]/div[@class="col-lg-4"]') as $li)
{
    ...
    $village_link = $base_site_link.($village_link[0]->getAttribute('href'));
    ...
    {
        reg_parse_error('plots not found', "Не удалось найти участки в поселке со сыллкой '$village_link'");
        continue;
    }
    //Запись имени и количества участков в поселке
    $village_arr['village_name'] = $main_page->query('div[@class = "field-name"]', $li)[0]->textContent;
    $village_arr['village_name'] = explode('"', $village_arr['village_name'])[1];
    ...
    $file_contest = file_get_contents("$base_site_link/gallery/gallery-$galery_file.html");
    if (!$file_contest)
        reg_parse_error('gallery not found', "Не удалось получить галерею поселка со ссылкой '$village_link'.");
    ...
    foreach($plots as $plot)
    {
        ...
        //Цена участка
        $plots_arr['price'] = $chars[2]->textContent;
        ...
        $plots_arr['area'] = $chars[1]->textContent;
        if ($plots_arr['area'] != null)
        {
            $plots_arr['area'] = preg_replace('/([^0-9,])/', '', $plots_arr['area']);
