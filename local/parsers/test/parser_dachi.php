...
const SITE_NAME = 'dachi_mo'; //Название сайта для получения настроек конкретного парсера из config.json
//$_SERVER["DOCUMENT_ROOT"] = '/var/www/u0428181/data/www/olne.ru/local/parsers/'.SITE_NAME.'/';
...
foreach($villages_links as $village_num => $li)
{
    ...
    $village_arr['village_name'] = preg_replace('/[\t\n]/', '', $vilages_names[$village_num]->textContent);
    ...
    foreach($plots as $plot)
    {
        ...
        {
            //Если участок публичный, убрать его из общего числа участков
            if ($plot->getAttribute('status') == 'pub')
                $village_arr['plots_count']--;
        ...
        //Цена участка
        $plots_arr['price'] = $plot->childNodes[$plot->childNodes->length - 2]->textContent;
        if ($plots_arr['price'] != null)
            $plots_arr['price'] = (int)preg_replace('/\D/', '',  $plots_arr['price']);
        ...
        $plots_arr['area'] = $plot->childNodes[$plot->childNodes->length - 4]->textContent;
        if ($plots_arr['area'] != null){
            $plots_arr['area'] = preg_replace('/([^0-9][^0-9])/', '', $plots_arr['area']);
        ...
        if ($buf->length > 0)
        {
            ...
            if ($village_link != 'https://dachi-mo.ru/ghk_lievobieriezhnyi')
                ...
                foreach ($buf as $village_link_add)
                    $plots_arr['links'] .= $base_site_link.$village_link_add->getAttribute('data-src').';';
...
function get_config() //Получить текущие настройки парсера из файла config.json
{
    ...
    if (!$config_str)
    {
        ...
        "'.SITE_NAME.'":
        {
            ...
            "base_site_link" : "https://dachi-mo.ru",
