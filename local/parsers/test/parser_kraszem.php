...
const SITE_NAME = 'kraszem'; //Название сайта для получения настроек конкретного парсера из config.json
...
$dom = new DOMDocument();
$dom->loadHTML($parser->open_main_page("$parser->base_site_link/cottages/"));
$main_page = new DOMXPath($dom);

//Получение всех поселков страницы
$villages = $main_page->query('//section[@class="catalog row"][1]/figure/figcaption/a');
foreach($villages as $village_num => $village_inf)
{
    ...
    $parser->add_village_info('name', $village_inf->textContent);
    $genplan = $village_page->query('//iframe[@class="village_map_frame"]')[0];
    if (!$genplan)
    {
        output_error("Генплан поселка $village_inf->textContent не найден.");
        continue;
    }
    ...
    $plots_xpath = new DOMXPath($plots_dom);

    $plots = $plots_xpath->query('//div[@class="wrapper"]/script')[0];
    $plots = explode('village.map_objects={', $plots->textContent)[1];
    $plots = substr($plots, 0, strpos($plots, ';'));
    $plots = explode('{', $plots);
    ...
    //Получение галлереи
    $gallery = $village_page->query('//div[@class="item__img"]');
    foreach ($gallery as $img)
        $parser->add_village_info('link', $parser->base_site_link.(substr(explode('url(', $img->getAttribute('style'))[1], 0, -1)));
    ...
    foreach($plots as $plot)
    {
        ...
        $parser->add_plot_info('num_id', get_js_attr($plot, 'number'));
        $parser->add_plot_info('price', (int)((float)preg_replace('/[^0-9.]/', '', get_js_attr($plot, 'total_cost'))));
        $parser->add_plot_info('area', get_js_attr($plot, 'area'));
...
function get_js_attr(string &$arr, string $name)
{
    $attr = substr($arr, strpos($arr, $name) + strlen($name));
    $attr = substr($attr, 1, strpos($attr, ',') - 1);
