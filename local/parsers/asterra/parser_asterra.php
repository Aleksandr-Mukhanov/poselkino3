...
$dom = new DOMDocument();
$dom->loadHTML($parser->open_main_page("$parser->base_site_link/projects"));
...
foreach($villages as $village_index => $li)
{
    ...
    $parser->add_village_info('name', $li->childNodes[1]->textContent);
    ...
    foreach($plots as $plot)
    {
        ...
        $file_contest = $parser->open_plot_page($village_link."items/$num/");
        if (!$file_contest)
            continue;
        ...
        //Номер участка
        $parser->add_plot_info('num_id', substr($plot->getAttribute('id'), 1));
        $parser->add_plot_info('price', $plot_page->query('//div[@class = "price"]')[0]->firstChild->textContent);
        ...
        foreach ($characteristics as $char)
        {
            ...
            case "Площадь участка":
                $parser->add_plot_info('area', $char->getElementsByTagName('dd')[0]->textContent);
                break;
            case "Кадастровый номер":
                $parser->add_plot_info('cadastral_number', $char->getElementsByTagName('dd')[0]->textContent);
                break;
            }
        }
        ...
        if ($buf->length > 0)
            foreach ($buf as $village_link_add)
                $parser->add_plot_info('link', $parser->base_site_link.$village_link_add->getAttribute('href'));
        else
            $parser->reg_parse_error('gallery images not found', "Изображения в галерее не найдены - участок $num в ".$parser->getVillageProperty('name'));
