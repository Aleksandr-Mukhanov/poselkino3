...
foreach($main_page->query('//section[@class="container village"]/ul/li') as $village_index => $li)
{
    ...
    //Запись имени
    $parser->add_village_info('name', $main_page->query('div[@class = "sale__item-content"]/a/h3', $li)[0]->textContent);
    ...
    if ($parser->getVillageProperty('name') == 'Резиденция Минино')
    ...
    foreach ($plots as $plot)
    {
        ...
        $parser->add_plot_info('num_id', $plot['NAME']);
        $parser->add_plot_info('price', $plot['PROPERTY_PRICE_CALC_VALUE']);//Цена
        $parser->add_plot_info('area', $plot['PROPERTY_PLOSHAD_VALUE']);//Площадь в сотках
