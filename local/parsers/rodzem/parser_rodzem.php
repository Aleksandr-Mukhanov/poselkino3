...
$dom = new DOMDocument();
$dom->loadHTML($parser->open_main_page($parser->base_site_link."/zemelnie-uchastki"));
...
foreach($villages as $village_num => $li)
{
    ...
    $parser->add_village_info('plots_count', $village_page->query('//ul[@class="listpanel-head"]/li[3]')[0]->textContent);

    // получим сотка от
    $sotka_ot = $village_page->query('//div[@class="priceWrap price_to"]/div[@class="price"]/span[1]/span[@class="textSecondary "]')[0];
    ...
    {
        set_time_limit(30);
        $file_contest = $parser->open_plot_page($parser->base_site_link.'/'.$add_page->getAttribute('href'));
        if (!$file_contest)
            continue;
    ...
    if ($buf->length > 0)
        foreach ($buf as $village_link_add)
            $parser->add_village_info('link', $parser->base_site_link.($village_link_add->getAttribute('data-thumb-image')));
    else
    ...
    foreach($plots as $plot)
    {
        ...
        $parser->add_plot_info('price', $plot->childNodes[5]->textContent);
        $parser->add_plot_info('num_id', $plot->childNodes[3]->textContent);
        $parser->add_plot_info('area', $plot->childNodes[6]->textContent);
        $parser->add_plot_info('cadastral_number', $plot->childNodes[8]->textContent);
    ...
    function add_plot_info($attribute, $info)
    {
        switch ($attribute)
        {
            case 'name':
                ...
                break;
            case 'num_id':
                $this->plot_arr['num_id'] = preg_replace('/^\s+|\s+$|\s+(?=\s)/', '', $info);
                break;
            case 'price':
                if ($info == null || $info == ''){
