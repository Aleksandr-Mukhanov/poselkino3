...
const SITE_NAME = 'dachi-mo'; //Название сайта для получения настроек конкретного парсера из config.json
...
$villages_links = $main_page->query('//a[@class ="middle js-goods-popup-link ul-goods-view-button-link "]');
$villages_links = array_slice(iterator_to_array($villages_links), 0, -1);
$vilages_names = $main_page->query('//div[@class ="js-goods-contenteditable h4"]/span');

foreach($villages_links as $village_num => $li)
{
    ...
    $village_link = $parser->base_site_link.$li->getAttribute('href');
    ...
    $plots = $village_page->query('//div[@class="popup"]');
    if ($plots->length == 0)
    {
        $parser->reg_parse_error('page_not_found', "Генплан поселка по ссылке $village_link не найден.");
        continue;
    }


    //Запись имени и количества участков данного поселка
    $parser->add_village_info('name', $vilages_names[$village_num]->textContent);
    $parser->add_village_info('plots_count', $plots->length);
    //Получение изображений в галерее
    $buf = $village_page->query('//div[@class = "ul-w-gallery-item ul-w-gallery-hover"]');
    if ($buf->length > 0)
    {
        if ($village_link != 'https://dachi-mo.ru/ghk_lievobieriezhnyi')
            foreach ($buf as $buf_)
                foreach ($village_page->query('img', $buf_) as $village_link_add)
                    $parser->add_village_info('link', $parser->base_site_link.$village_link_add->getAttribute('src'));
        else
            foreach ($buf as $village_link_add)
                $parser->add_village_info('link', $parser->base_site_link.$village_link_add->getAttribute('data-src'));
    }
    ...
    foreach($plots as $plot)
    {
        ...
        if ($plot->getAttribute('status') != 'free')
            continue;

        //Номер участка
        $num = (int)$plot->getAttribute('rel');
        $parser->add_plot_info('num_id', $num);

        $parser->add_plot_info('price', $plot->childNodes[$plot->childNodes->length - 2]->textContent);
        $parser->add_plot_info('area', $plot->childNodes[$plot->childNodes->length - 4]->textContent);
