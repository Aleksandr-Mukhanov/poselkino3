...
$_SERVER["DOCUMENT_ROOT"] = '/var/www/u0428181/data/www/olne.ru/dir/spb';
...
// получим шоссе SHOSSE
	...
	$propEnums = CIBlockPropertyEnum::GetList(
		["SORT"=>"ASC","VALUE"=>"ASC"],
		["IBLOCK_ID"=>7,"CODE"=>$codeProp]
	);
	while($enumFields = $propEnums->GetNext()){ // dump($enumFields);
		$xml_content .= '<url><loc>https://spb.poselkino.ru/poselki/'.$enumFields['XML_ID'].'-shosse/</loc><lastmod>'.date('c').'</lastmod><priority>1</priority></url>';
		$xml_content .= '<url><loc>https://spb.poselkino.ru/kupit-uchastki/'.$enumFields['XML_ID'].'-shosse/</loc><lastmod>'.date('c').'</lastmod><priority>1</priority></url>';
...
// получим районы REGION
	...
	while($enumFields = $propEnums->GetNext()){ // dump($enumFields);
		$xml_content .= '<url><loc>https://spb.poselkino.ru/poselki/'.$enumFields['XML_ID'].'-rayon/</loc><lastmod>'.date('c').'</lastmod><priority>1</priority></url>';
		$xml_content .= '<url><loc>https://spb.poselkino.ru/kupit-uchastki/'.$enumFields['XML_ID'].'-rayon/</loc><lastmod>'.date('c').'</lastmod><priority>1</priority></url>';
...
// Цена
	...
	foreach ($urlPrice as $key => $value) {
		$xml_content .= '<url><loc>https://spb.poselkino.ru'.$value.'</loc><lastmod>'.date('c').'</lastmod><priority>1</priority></url>';
	} // echo $xml_content;
...
// Площадь
	for ($i=2; $i <= 20; $i++) { // участки
		...
		$xml_content .= '<url><loc>https://spb.poselkino.ru/kupit-uchastki/'.$i.'-'.$urlSot.'/</loc><lastmod>'.date('c').'</lastmod><priority>1</priority></url>';
	}
	$xml_content .= '<url><loc>https://spb.poselkino.ru/kupit-uchastki/25-sotok/</loc><lastmod>'.date('c').'</lastmod><priority>1</priority></url>';
	for ($i=30; $i <= 100; $i+=10) {
		$xml_content .= '<url><loc>https://spb.poselkino.ru/kupit-uchastki/'.$i.'-sotok/</loc><lastmod>'.date('c').'</lastmod><priority>1</priority></url>';
	}
	for ($i=2; $i <= 20; $i++) { // дома
    if($i == 2 || $i == 3 || $i == 4){
      $nameSot = 'сотки'; $urlSot = 'sotki';
...
// получим статические страницы
  ...
  // dump($arDirList);
	...
	foreach ($arDirList as $key => $value) {
		$xml_content .= '<url><loc>https://spb.poselkino.ru'.$value['URL'].'</loc><lastmod>'.$value['DATE'].'</lastmod><priority>0.5</priority></url>';
	}
...
// получим девелоперов
	...
	// 	$xml_content .= '<url><loc>https://spb.poselkino.ru/developery/'.$arRes['UF_XML_ID'].'/</loc><lastmod>'.date('c').'</lastmod><priority>0.5</priority></url>';
	// } // dump($arDeveloper);
...
// получим инфоблоки
	...
	foreach ($arURL as $key => $value) {
		$xml_content .= '<url><loc>https://spb.poselkino.ru'.$value['URL'].'</loc><lastmod>'.$value['DATE'].'</lastmod><priority>0.8</priority></url>';
	}
...
// район до МКАД и газ
	foreach ($arRegion as $rayon => $value) {
		$xml_content .= '<url><loc>https://spb.poselkino.ru/kupit-uchastki/'.$rayon.'-rayon-gaz/</loc><lastmod>'.date('c').'</lastmod><priority>1</priority></url>';
		$xml_content .= '<url><loc>https://spb.poselkino.ru/poselki/'.$rayon.'-rayon-gaz/</loc><lastmod>'.date('c').'</lastmod><priority>1</priority></url>';
	}
// до МКАД газ
	for ($i=10; $i < 60; $i+=10) {
...
// стоимость
	...
	foreach ($urlPriceShosse as $price) {
		foreach ($onlyShosse as $shosse) {
			$xml_content .= '<url><loc>https://spb.poselkino.ru/kupit-uchastki/'.$shosse.'-shosse-'.$price.'/</loc><lastmod>'.date('c').'</lastmod><priority>1</priority></url>';
...
// коммуникации и шоссе
	...
	foreach ($commun2 as $commun) {
		foreach ($onlyShosse as $shosse) {
			$xml_content .= '<url><loc>https://spb.poselkino.ru/kupit-uchastki/'.$shosse.'-shosse-'.$commun.'/</loc><lastmod>'.date('c').'</lastmod><priority>1</priority></url>';
			$xml_content .= '<url><loc>https://spb.poselkino.ru/poselki/'.$shosse.'-shosse-'.$commun.'/</loc><lastmod>'.date('c').'</lastmod><priority>1</priority></url>';
		}
	}
// коммуникации и МКАД
	foreach ($commun2 as $commun) {
...
// запись в файл sitemap-tegi-pages.xml
	...
	foreach ($arSitemapPage as $sitemap) {
		$xml_content .= '<sitemap><loc>https://spb.poselkino.ru/sitemap/'.$sitemap.'</loc></sitemap>';
	}
