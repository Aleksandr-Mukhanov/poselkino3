...
}elseif($mkadKM){ // если выбор расстояние от КАД

  $arMetaInfo = getMetaInfo($arrFilter);

  switch ($domPos) {
    case 'noDom': // Участки
      $SEO_text = '<p>В данном разделе представлены ▶ земельные участки в '.$mkadKM.' км от КАД. Здесь представлено ▶ '.$arMetaInfo['cntPos'].' '.$areaName.' с независимыми отзывами и рейтингом. Вы можете выбрать самые доступные цены на землю в '.$mkadKM.' км от КАД ▶ стоимость варьируется от '.$arMetaInfo['minPrice'].' рублей до '.$arMetaInfo['maxPrice'].' рублей.</p>
      <p>★ Особенности проекта Poselkino ★ - для каждого участка мы подбираем данные:</p>';
      break;
    case 'withDom': // Дома
      $SEO_text = '<p>В данном разделе представлены ▶ коттеджи и дома в '.$mkadKM.' км от КАД. Здесь представлено ▶ '.$arMetaInfo['cntPos'].' '.$houseName.' с независимыми отзывами и рейтингом. Вы можете выбрать самые доступные цены на коттеджи в '.$mkadKM.' км от КАД ▶ стоимость варьируется от '.$arMetaInfo['minPrice'].' рублей до '.$arMetaInfo['maxPrice'].' рублей.</p>
      <p>★ Особенности проекта Poselkino ★ - для каждого загородного дома мы подбираем данные:</p>';
      break;
    default: // поселки
      $SEO_text = '<p>В данном разделе представлены ▶ коттеджные поселки в '.$mkadKM.' км от КАД. Здесь представлено ▶ '.$arMetaInfo['cntPos'].' '.$vilName.' с независимыми отзывами и рейтингом. Вы можете выбрать самые доступные цены на коттеджи в поселках на расстоянии '.$mkadKM.' км от КАД ▶ стоимость варьируется от '.$arMetaInfo['minPrice'].' рублей до '.$arMetaInfo['maxPrice'].' рублей.</p>
      <p>★ Особенности проекта Poselkino ★ - для каждого объекта мы подбираем данные:</p>';
      break;
  }
  $SEO_text_add = true;
