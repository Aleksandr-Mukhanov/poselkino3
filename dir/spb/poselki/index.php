...
$newTitle = false;
$newDesc = false;
$h1 = 'Поселки в Ленинградской области';
$h2 = '<h2 class="h2">Земельные участки под дом и дачу с хорошим месторасположением</h2>';
$SEO_text = '<p>База коттеджных и дачных поселков в Ленинградской области. Каталог позволяет найти участки по нужным шоссе и районам, по площади и стоимости, по удаленности от '.ROAD.' и коммуникациям. Каждый поселок имеет свой рейтинг, оценку пользователей и отзывы.</p><p>Вы можете узнать всю необходимую информацию об интересующем вас поселке, не выходя из дома. На сайте есть фото и видео обзоры поселков, юридическая информация и объекты неблагоприятной экологии.</p>';
$urlAll = '/poselki/';
...
if ($pagen && $pageTitleDesc) { // дописываем страницу в пагинации
    if ($pageTitle && $pageDesc) { // свои
        $newTitle = $pageTitle . ' - страница ' . $pagen;
        $h1 = $pageDesc . ' - страница ' . $pagen;
        $newDesc = $pageDesc . ' - страница ' . $pagen . ' - сайт Поселкино';
    } else { // универсальные
        $h1 = $pageTitleDesc . ' - Страница ' . $pagen;
        $newTitle = $pageTitleDesc . ' - Страница ' . $pagen . ' | сайт Посёлкино';
        $newDesc = $pageTitleDesc . ' - Страница ' . $pagen . '. Сайт Посёлкино - вся загородная недвижимость в Ленинградской области';
    }
}
?>
            ...
            <div class="show-mobile-filter" style="display: none;">
                <? $APPLICATION->IncludeComponent(
                    ...
                    array(
                        ...
                        "XML_EXPORT" => "N",
                        "COMPONENT_TEMPLATE" => "poselkino_spb"
                    ),
                    false
                ); ?>
                <? // dump($arrFilter);
    ...
    <div class="block-page">
        <div class="container">
            ...
            <div class="block-page__offer" id="special_offers">
                ...
                if ($arrFilter["=PROPERTY_REGION"]) $addFilter["=PROPERTY_REGION"] = $arrFilter["=PROPERTY_REGION"];
                if ($arrFilter["=PROPERTY_SHOSSE"]) $addFilter["=PROPERTY_SHOSSE"] = $arrFilter["=PROPERTY_SHOSSE"];
